...
<p align="center">
  <a href="https://github.com/lolokeksu/F2FS-Guardian/releases/tag/v1.1"><img alt="Release v1.1" src="https://img.shields.io/badge/release-v1.1-0ea5e9"></a>
...
**v1.1 is the current stable release by Lolokeksu.**

v1.1 keeps the conservative F2FS policy from v1 and adds compatibility and interface improvements. The kernel GC modes, safety thresholds and ownership rules are unchanged.
...
The v1.1 compatibility and interface changes passed static and mocked tests. KernelSU Next, SukiSU and other device-specific environments must still run `self-test` after installation.

## What is new in v1.1

- added a multi-source `/data` filesystem detector for KernelSU Next and other isolated installer environments;
- installation no longer treats an unavailable filesystem result as proof that `/data` is incompatible;
- runtime verification remains strict and blocks kernel writes unless F2FS and all required nodes are confirmed;
- added exact `major:minor` to block-device and F2FS-instance resolution;
- removed the unsafe fallback that selected the first available F2FS instance;
- added a bilingual Russian/English terminal menu with persistent language selection;
- added short commands such as `f2g`, `f2status`, `f2doctor`, `f2start` and `f2stop`;
- added a detailed `doctor` diagnostic command;
- added a boot-completion wait before the daemon starts;
- fixed the endless `Invalid selection` loop in Manager Action windows without interactive stdin;
- Manager Action now displays a status dashboard and directs the user to `f2g` in Termux or ADB for interactive control.
...
- Magisk 20.4+, KernelSU, KernelSU Next, SukiSU-compatible KernelSU environments or APatch;
...
2. Download `F2FS-Guardian-v1.1.zip` from the GitHub Releases page.
...
# Open the bilingual terminal menu
...
After reboot, the systemless command wrappers provide a shorter interface:
...
`f2start` enables automatic maintenance. It does not immediately force a GC session. `f2request` places a bounded request in the queue and still obeys all safety conditions.

Termux is optional for background operation. It is required only when the user wants an interactive terminal on devices whose Manager Action window does not provide standard input.

## Manager Action behavior

Some KernelSU Next and SukiSU Manager Action windows execute `action.sh` without an interactive standard-input stream. A numbered `read` loop cannot work reliably in that environment.

F2FS Guardian therefore:

- opens the interactive menu only when a real TTY is available;
- otherwise prints a localized status dashboard;
- exits cleanly instead of repeating `Invalid selection`;
- directs the user to run `f2g` from Termux or an ADB shell.
...
module/system/bin/              Short command wrappers
